...
class IRtcEngineEventHandler {
  ...
  */
  ...
  /** Occurs when startServerSuperResolution API result is obtained
   *
   *
   * @brief startServerSuperResolution callback
   *
   * @param httpStatusCode http status code, 200 when success
   * @param reason reason when failed
   * @param dstImagePath image path, it is jpeg format
   * @param width image width
   * @param height image height
   */
  virtual void onServerSuperResolutionResult(int httpStatusCode, int errCode, const char* errReason, const char* dstImagePath, int width, int height) {
    (void)httpStatusCode;
    (void)errCode;
    (void)errReason;
    (void)dstImagePath;
...
class IRtcEngine : public agora::base::IEngineBase {
  ...
  */
  ...
  virtual int applyVideoEncoderMirrorToRemote(uid_t uid, VIDEO_MIRROR_MODE_TYPE mirrorMode) = 0;

  virtual int applyVideoOrientationToRemote(uid_t uid, VIDEO_ORIENTATION orientation) = 0;

  /**
   * Enables/Disables multi renderer and sets the options.
   *
   * @note
   * Ensure that you call this method before calling the `setupRemoteVideo` `setupLocalVideo`
   * `setLocalVideoMirrorMode` `setRemoteRenderMode` `setLocalRenderMode` method.
   * @param enabled Sets whether or not to enable multi renderers:
   * - true: enables multi renderers.
   * - false: disables multi renderers.
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
   ...
   *
   * @param canvas The local video view setting: VideoCanvas.
   * @param mode The local video view setup mode: add, remove. If you add the same view twice,
   * the first view will be overwritten.
   *
   * @return
   * - 0: Success.
   * - < 0: Failure.
...
///@endcond
  ...
  */
   ...
   * - < 0: Failure.
   */
  virtual int setLocalRenderMode(media::base::RENDER_MODE_TYPE renderMode, VIDEO_MIRROR_MODE_TYPE mirrorMode, VIDEO_SOURCE_TYPE sourceType = VIDEO_SOURCE_CAMERA_PRIMARY) = 0;
  /**
  ...
  */
  ...
  virtual int enableFishEyeCorrection(bool enabled, const FishEyeCorrectionParams& params) = 0;

  /**
   * @brief save current time video frame to jpeg file
   *
   * @param uid
   * @param filePath users make sure that the dir of filePath exists, if not exists, API will fail
   * @param left
   * @param top
   * @param right
   * @param bottom
   ...
   * - <0 : Failure.
   */
  virtual int takeSnapshot(uid_t uid, const char* filePath, float left=0.0, float top=0.0, float right=1.0, float bottom=1.0) = 0;

  /**
   * @brief start server super resolution, only be used after joinChannel with token
   * only can once before callback onServerSuperResolution is called
   *
   * @param token token of current uid, generated by App server using appId, appCertificate and uid, can't be empty
   * @param srcImagePath source image file path, image content must be jpeg format, min 128*128, max 960*540(or 540*960)
   * @param dstImagePath destination image file path that sdk writes to, image content will be jpeg format, users make sure that the dir of dstImagePath exists
   * @param scale value in (1.0, 4.0], if <= 1.0, scale will be set to 2.0, if > 4.0, scale will be set to 4.0
   * @param timeoutSeconds, if < 3, timeoutSeconds will be set to 3
   * @return int
   * - 0 : Success.
   * - <0 : Failure.
   */
  virtual int startServerSuperResolution(const char* token, const char* srcImagePath, const char* dstImagePath, float scale, int timeoutSeconds) = 0;
