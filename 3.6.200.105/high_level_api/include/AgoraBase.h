...
/**
 * The definition of the VideoCropArea struct, which contains the area of the video which should be displayed on renderer .
 */
struct VideoCropArea {
  ...
  VideoCropArea() : left(.0F), top(.0F), right(.0F), bottom(.0F) {}
  VideoCropArea(float l, float t, float r, float b) : left(l), top(t), right(r), bottom(b) {}
  ...
  bool isValid() const {
    return (left >= 0.0F && right <= 1.0F) && (top >= 0.0F && bottom <= 1.0F);
  }
};
...
enum BRIGHTNESS_CORRECTION_MODE {
    AUTO_MODE = 0,
    MANUAL_MODE = 1,
};

enum TrapezoidAutoCorrectionResult {
    RESULT_OK = 0,
    RESULT_FAILED = -1,
};
...
/** FishEye options
*/
struct FishEyeCorrectionParams {
  /* [0.4, 0.6] */
  float xCenter;
  /* [0.4, 0.6] */
  float yCenter;
  ...
  bool isValid() const {
    return (xCenter >= 0.4F && xCenter <= 0.6F) &&
           (yCenter >= 0.4F && yCenter <= 0.6F) &&
           (scaleFactor >= 1.0F && scaleFactor <= 5.0F) &&
           (focalLength >= 20 && focalLength <= 40) &&
           (polFocalLength >= 20 && polFocalLength <= 40) &&
           (splitHeight >= 1.0F && splitHeight <= 5.0f);
  }
};
